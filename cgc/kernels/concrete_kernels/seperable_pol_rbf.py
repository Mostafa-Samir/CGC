...
class SeprablePolynomialParameters(BaseKernelParameters):
    ...
    rbf_scale: KernelParameter = KernelParameter(1.0, learnable=False)


@KernelsFactory.register("separable-polynomial-rbf", SeprablePolynomialParameters)
class PolynoSeprablePolynomialmialKernel(BaseKernel):
    ...
    def _eval(self, x, y, **params):
        ...
        scale = params.get("rbf_scale")
        
        return self._poly_eval(x[p_start: p_end], y[p_start:p_end], constant, exponent) + self._rbf_eval(x[q_start:], y[q_start:], scale)